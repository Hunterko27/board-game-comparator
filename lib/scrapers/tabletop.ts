...
export class TabletopScraper implements Scraper {
    name = 'Tabletop';

    async search(query: string): Promise<SearchResult[]> {
        const url = `https://www.tabletop.cz/search-engine.htm?slovo=${encodeURIComponent(query)}&search_submit=&hledatjak=2`;
        ...
        try {
            ...
            $('div.product').each((_, element) => {
                ...
                if (imageUrl && !imageUrl.startsWith('http')) {
                    imageUrl = `https://www.tabletop.cz${imageUrl}`;
                }
                ...
                if (name && !isNaN(price) && link) {
                    results.push({
                        ...
                        currency: 'CZK',
                        availability,
                        link: link.startsWith('http') ? link : `https://www.tabletop.cz${link}`,
                        shopName: 'Tabletop',
                        imageUrl
                    });
                }
            ...
            console.error('Error scraping Tabletop:', error);
        }
