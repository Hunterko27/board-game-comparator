...
export class CechHracuScraper implements Scraper {
    name = 'CechHracu';

    async search(query: string): Promise<SearchResult[]> {
        const url = `https://www.cechhracu.cz/vyhledavani/?string=${encodeURIComponent(query)}`;
            ...
            $('div.product').each((_, element) => {
                ...
                if (name && !isNaN(price) && link) {
                    results.push({
                        ...
                        currency: 'CZK',
                        availability,
                        link: link.startsWith('http') ? link : `https://www.cechhracu.cz${link}`,
                        shopName: 'CechHracu',
                        imageUrl: imageUrl ? (imageUrl.startsWith('http') ? imageUrl : `https://www.cechhracu.cz${imageUrl}`) : undefined
                    });
                }
            });
        } catch (error) {
            console.error('Error scraping CechHracu:', error);
        }
